...
void cLEDMatrixBase::VerticalMirror() {
  ...
  for (y = (m_Height / 2) - 1, yy = ((m_Height + 1) / 2); y >= 0; --y, ++yy) {
    for (x = m_Width - 1; x >= 0; --x) m_LED[mXY(x, yy)] = m_LED[mXY(x, y)];
  }
}
...
void cLEDMatrixBase::TriangleTopMirror(bool FullHeight) {
  ...
  if (m_Width > m_Height)
    ...
    MaxXY = m_Width - 1;
  if (!FullHeight) MaxXY /= 2;
  for (y = 1; y <= MaxXY; ++y) {
    for (x = 0; x < y; ++x) m_LED[mXY(y, x)] = m_LED[mXY(x, y)];
  }
}
...
void cLEDMatrixBase::DrawLine(int16_t x0, int16_t y0, int16_t x1, int16_t y1,
                              CRGB Col) {
  int16_t dx = x1 - x0;
  int16_t dy = y1 - y0;
  if (abs(dx) >= abs(dy)) {
    ...
    if (dx == 0) {
      ...
      if (dx >= 0) {
        for (; x0 <= x1; ++x0, y += f) (*this)(x0, (y >> 16)) = Col;
      } else {
        for (; x0 >= x1; --x0, y += f) (*this)(x0, (y >> 16)) = Col;
      }
    }
    ...
    if (dy >= 0) {
      for (; y0 <= y1; ++y0, x += f) (*this)((x >> 16), y0) = Col;
    } else {
      for (; y0 >= y1; --y0, x += f) (*this)((x >> 16), y0) = Col;
    }
  }
...
void cLEDMatrixBase::DrawRectangle(int16_t x0, int16_t y0, int16_t x1,
                                   int16_t y1, CRGB Col) {
...
void cLEDMatrixBase::DrawFilledRectangle(int16_t x0, int16_t y0, int16_t x1,
                                         int16_t y1, CRGB Col) {
  int16_t y = min(y0, y1);
  for (int16_t c = abs(y1 - y0); c >= 0; --c, ++y) DrawLine(x0, y, x1, y, Col);
}

void cLEDMatrixBase::DrawFilledCircle(int16_t xc, int16_t yc, uint16_t r,
                                      CRGB Col) {
  int16_t x = r;
  int16_t y = 0;
  int16_t e = 1 - x;

...
class cLEDMatrixBase {
 ...
 public:
  ...
  void SetLEDArray(
      struct CRGB *pLED);  // Only used with externally defined LED arrays
  ...
  void DrawFilledRectangle(int16_t x0, int16_t y0, int16_t x1, int16_t y1,
                           CRGB Col);
...
template <int16_t tWidth, int16_t tHeight, MatrixType_t tMType,
          int16_t tXMult = 0, int16_t tYMult = 0>
class cLEDMatrix : public cLEDMatrixBase {
 private:
  static const int16_t m_absWidth =
      (tWidth * ((tWidth < 0) * -1 + (tWidth > 0)));
  static const int16_t m_absHeight =
      (tHeight * ((tHeight < 0) * -1 + (tHeight > 0)));
  struct CRGB
      p_LED[(m_absWidth * m_absHeight * ((tXMult == 0) && (tXMult == 0))) +
            ((tXMult != 0) && (tXMult != 0))];  // Will always be at least 1
 ...
 public:
  ...
  void SetLEDArray(struct CRGB *pLED) { m_LED = pLED; }
  virtual uint16_t mXY(uint16_t x, uint16_t y) {
    if (tWidth < 0) x = (m_absWidth - 1) - x;
    if (tHeight < 0) y = (m_absHeight - 1) - y;
 ...
 private:
  ...
  void HPWSL(void) {
    ...
    for (int16_t y = m_absHeight; y > 0; --y, ++i) {
      for (int16_t x = m_absWidth - 1; x > 0; --x, ++i) p_LED[i] = p_LED[i + 1];
      p_LED[i] = CRGB(0, 0, 0);
    }
  }
  void HNWSL(void) {
  ...
  void VPWSL(void) {
    ...
    for (int16_t x = m_absWidth - 1; x > 0; --x) {
      for (int16_t y = m_absHeight; y > 0; --y) p_LED[i++] = p_LED[j++];
    }
    for (int16_t y = m_absHeight; y > 0; --y) p_LED[i++] = CRGB(0, 0, 0);
  }
  void VNWSL(void) {
  ...
  void HPHSD(void) {
    ...
    for (int16_t y = m_absHeight - 1; y > 0; --y) {
      for (int16_t x = m_absWidth; x > 0; --x) p_LED[i++] = p_LED[j++];
    }
    for (int16_t x = m_absWidth; x > 0; --x) p_LED[i++] = CRGB(0, 0, 0);
  }
  void HNHSD(void) {

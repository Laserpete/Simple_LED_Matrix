...
// AND leds[-1] is now a legitimate and safe alias for
// leds_plus_safety_pixel[0]. leds_plus_safety_pixel[0] aka leds[-1] is now your
// "safety pixel".
//
...
void rainbow(byte startHue8, int8_t yHueDelta8, int8_t xHueDelta8) {
  byte lineStartHue = startHue8;
  for (byte y = 0; y < kMatrixHeight; y++) {
    lineStartHue += yHueDelta8;
    byte pixelHue = lineStartHue;
...
void confetti() {
  ...
  leds[pos] += CHSV(0 + random8(255), 200, 255);
}
...
void loop() {
  ...
  rainbow(ms / 65536, yHueDelta32 / 32768, xHueDelta32 / 32768);
  // confetti();
  if (ms < 5000) {
